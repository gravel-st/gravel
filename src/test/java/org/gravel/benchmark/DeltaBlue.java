...
class Strength {
	...
	public final static Strength strongPreferred = new Strength(1,
			"strongPreferred");
	public final static Strength preferred = new Strength(2, "preferred");
	public final static Strength strongDefault = new Strength(3,
			"strongDefault");
...
class Variable {
	...
	public ArrayList<Constraint> constraints; // normal constraints that
												// reference me
	...
	protected void chooseMethod(int mark) {
		if (v1.mark == mark)
			direction = v2.mark != mark
					&& Strength.stronger(strength, v2.walkStrength) ? forward
					: nodirection;

		if (v2.mark == mark)
			direction = v1.mark != mark
					&& Strength.stronger(strength, v1.walkStrength) ? backward
					: nodirection;

		// If we get here, neither variable is marked, so we have a choice.
		if (Strength.weaker(v1.walkStrength, v2.walkStrength))
			direction = Strength.stronger(strength, v1.walkStrength) ? backward
					: nodirection;
		else
			direction = Strength.stronger(strength, v2.walkStrength) ? forward
					: nodirection;
	}
...
class ScaleConstraint extends BinaryConstraint {
	...
	public ScaleConstraint(Variable src, Variable scale, Variable offset,
		...
		// Curse this wretched language for insisting that constructor
		// invocation
...
class Planner {
	...
	protected void addConstraintsConsumingTo(Variable v,
			ArrayList<Constraint> coll) {
		Constraint determiningC = v.determinedBy;
...
public class DeltaBlue /* implements Benchmark */{
	...
	private void benchmark(int iterations, int n) {
		...
		System.out.println("DeltaBlue\tJava\t" + iterations + "x\t"
	...
	public static void standardBenchmark() {
		new DeltaBlue().benchmark(100, 100);
	}

	public static void longBenchmark() {
		new DeltaBlue().benchmark(10, 20000);
	}
}
